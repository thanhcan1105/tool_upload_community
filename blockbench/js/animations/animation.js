...
window.loadAnimationFromApp = async function loadAnimationFromApp(data) {
	await Modes.options.animate.select();

	for (let file of data) {
		let keys = Object.keys(Object.values(file)[1])[0];
		// let names = keys.replaceAll('controller.animation.', '').replaceAll('animation.', '');
		// let name = names.split('.')[0];
		await Animator.importFile({
			name: keys + '.json',
			path: keys + '.json',
...
window.saveAnimationToApp = async function saveAnimationToApp() {
	console.log('saveAnimationToApp');

	let paths = [];
	let controller_paths = [];

	let animationData = {};
	let animationControllerData = {};
	...
	paths.forEach(path => {
		animationData = Animator.exportAnimationFile(path);
	})
	controller_paths.forEach(path => {
		animationControllerData = Animator.exportAnimationControllerFile(path);
	})
