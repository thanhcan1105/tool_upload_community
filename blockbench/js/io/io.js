...
window.loadTextureApp = function loadTextureApp(fileName, fileContent) {
...
window.loadModelFromApp = async function loadModelFromApp(modelData) {
	...
	if (ModelProject.all.length > 1) {
		ModelProject.all[0].close();
	}
};

window.saveDataModelToApp = function saveDataModelToApp() {
	let data = "";
	let dataModel = Format.codec.compile();
	let dataTexture = Texture.getDefault().source;

	data = `{"model":{"file_name":"model.json","data":${dataModel}},"texture":{"file_name":"texture.png","data":"${dataTexture}"}}`
	return data.toString();
};

function saveDataToWeb() {
	console.log('saveDataToWeb');

	let data = [
		{
			"file_name": "model.json",
			"type": "model",
			"data": Format.codec.compile()
		},
		{
			"file_name": "texture.png",
			"type": "texture",
			"data": Texture.all[0].source
		}
	];
	console.log('Data To App: ' + data);

	window.flutter_inappwebview.callHandler('saveData', data);
}

function loadModelFromWeb() {
	let modelData = [
		{
			"file_name": "model.json",
			"type": "model",
		...
		},
		{
			"file_name": "texture.png",
			"type": "texture",
			"data": "",
		}
	];
	...
	for (let i = 0; i < modelData.length; i++) {
		...
		if (modelData[i]['type'].includes("model")) {
			loadModelApp({ path: modelData[i]['file_name'], no_file: true }, JSON.parse(JSON.stringify(modelData[i]['data'])));
		} else if (modelData[i]['type'].includes("texture")) {
			loadTextureApp(modelData[i]['file_name'], "data:image/png;base64," + modelData[i]['data']);
		}
	}
}

window.loadNewModelApp = function loadNewModelApp(modelData) {
	let objectData = {
		'data': [
			{
				"file_name": "model.json",
			...
			},
			{
				"file_name": "data.png",
				"type": "texture",
				"data": Texture.getDefault().source.replaceAll("data:image/png;base64,", "")
			},
		]
	};

	loadModelFromApp(objectData);
}
	...
	new Action('open_model', {
		...
		click: function () {
			...
			Blockbench.import({
				...
				files.forEach(file => {
					loadModelFile({ path: 'model.json', no_file: true, content: file.content });
				})
			})
		}
	})
		...
		click: function () {
			if (isApp) {
				...
				if (Format.codec && Format.codec.export) {
					Format.codec.export()
					// window.flutter_inappwebview.callHandler('funcSaveModel', Format.codec.compile());
				}
			}
		}
	})
