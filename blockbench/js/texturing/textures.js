...
class Texture {
	...
	openMenu() {
		...
		let form = {
			...
			'render_options': '_',
			render_mode: {
				label: 'menu.texture.render_mode', type: 'select', value: this.render_mode, options: {
		...
		if (Format.id == 'free') {
			Object.assign(form, {
				render_sides: {
					label: 'settings.render_sides', type: 'select', value: this.render_sides, options: {
		...
		if (Format.texture_mcmeta) {
			Object.assign(form, {
				...
				frame_order_type: {
					label: 'dialog.texture.frame_order_type', type: 'select', value: this.frame_order_type, options: {
	...
	resizeDialog() {
		...
		let dialog = new Dialog({
			...
			form: {
				...
				fill: {
					label: 'dialog.resize_texture.fill', type: 'select', default: 'transparent', options: {
	...
	exportEmissionMap() {
		new Dialog({
			...
			form: {
				format: {
					label: 'dialog.export_emission_map.format', type: 'select', options: {
