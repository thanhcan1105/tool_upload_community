...
// window.undoFromApp = 
window.undoFromApp = function undoFromApp() {
	if (Undo.history.length > 0 && Undo.index > 1) {
		Undo.undo();
	}
}
window.redoFromApp = function redoFromApp() {
	if (Undo.history.length > 0 && Undo.index < Undo.history.length - 1) {
		Undo.redo();
	}
}